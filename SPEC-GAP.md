...
- ~~**VR-5** Large-attachment AEAD AAD uses Go-struct field order, not canonical alphabetical.~~ Landed. `largeattachment.AdditionalData` now routes through `internal/canonical.Marshal` so the AAD bytes are spec-canonical (keys sorted lexicographically) regardless of Go struct declaration order.
- ~~**VR-6** Hybrid Kyber+X25519 wire layout placed X25519 first.~~ Landed. `crypto/kem_hybrid.go` now serializes the hybrid public key, private key, and KEM ciphertext as `kyber || x25519` to match `ENVELOPE.md` §4.4.1 and the cross-language vectors. Earlier releases were `x25519 || kyber`, which produced byte-incompatible PQ artifacts with every other SEMP implementation. Wire-breaking change at the PQ layer.
- ~~**VR-7** PQ KEM was Kyber Round 3, not FIPS 203 ML-KEM-768.~~ Landed. `crypto/kem_hybrid.go` swapped `circl/kem/kyber/kyber768` for `circl/kem/mlkem/mlkem768`. Same constant sizes (1184/2400/1088/32) so the public surface was unaffected, but the underlying algorithm now matches the NIST-standardized final ML-KEM-768 the spec calls for. Wire-breaking change at the PQ layer (Round 3 ciphertexts and ML-KEM ciphertexts under the same seed differ).

All seven VR findings (VR-1..VR-7) are closed. The vectors-runner verifies 89 of 89 cross-language vectors byte-for-byte against semp-go; zero skips.
